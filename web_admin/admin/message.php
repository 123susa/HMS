<?php
  ...
  if (isset($_SESSION["AEmail"])){
    ...
    $contact="SELECT * FROM `contact` WHERE `check`=0 ORDER BY `sendtime` DESC";
    $conres=mysqli_query($con,$contact);
  }
  else{
    ...
    <div class="row">
            <div class="col-12-md">
                <div class="appoint-but">
                    <button class="btn btn-outline-success m-2" onclick="showDiv('contact');hideDiv('mail');">Contact List</button>
                    <!-- <button class="btn btn-outline-success m-2" onclick="showDiv('mail');hideDiv('contact');">Send Mail</button> -->
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12 " id="contact">
                ...
                <table class="table">
                  ...
                  <?php
                    $n=1;
                        while($conrow=mysqli_fetch_assoc($conres)){
                            // $_SESSION['fullid']="{$conrow['sno']}";
                            echo "
                            <tr>
                                <td>$n</td>
                                <td>{$conrow['Name']}</td>
                                <td id='textbox'>{$conrow['Email']}</td>
                                <td>{$conrow['Phone']}</td>
                                <td>{$conrow['Msg']}</td>
                                <td><form action='../php/check.php' method='post'><button name='conshow' class='btn btn-success' value='{$conrow['sno']}'>Send</button></form></td>
                                <td><form action='../php/deleten&l.php' method='post'><button name='condel' class='btn btn-danger' value='{$conrow['sno']}'>Delete</button></form></td>
                            </tr>
                            ";
                            $n++;
                        }
            ...
            <!-- <div id="mail" class="col-md-12 hidden">
            Button trigger modal
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#full">
                  Launch demo modal
                </button>

                Modal
                <div class="modal fade" id="full" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
