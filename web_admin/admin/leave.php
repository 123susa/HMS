<?php
  ...
  if (isset($_SESSION["AEmail"])){
    ...
    $dapart="SELECT * FROM `department` WHERE `Dep_id`='{$row['Host_dep']}'";
    $ddres=mysqli_query($con,$dapart);
    $ddrow=mysqli_fetch_assoc($ddres);

    $leave="SELECT * FROM `leave_schedule` WHERE `Doctor_id`={$row['sno']} AND `checkcom`!=3 ";
    $leaveres=mysqli_query($con,$leave);
    ...
    <div class="container">
        ...
        <div class="col-12-md">
                <div class="appoint-but">
                    <button class="btn btn-outline-success m-2" onclick="showDiv('list');hideDiv('leave');">Leave List</button>
                    <button class="btn btn-outline-success m-2" onclick="showDiv('leave');hideDiv('list');">Leave Form</button>
                </div>
            </div>
        </div>
        <div class="row">
            <div id="list">
                ...
                <div class="col-md-12 mt-3">
                    <table class="table">
                      ...
                      $n=1;
                        while($leave=mysqli_fetch_assoc($leaveres)){
                            if($leave['checkcom'] == 1){
                                $status='Completed';
                            }else{
                                $status= 'Pending';
                        ...
                        <tr>
                            ...
                            <td>{$drow['Account_type']}</td>
                            <td>{$leave['Day']} days</td>
                            <td>{$leave['From_Date']}</td>
                            <td>{$leave['To_Date']}</td>
                            <td>$status</td>
                            <td><form action='../php/check.php' method='post'><button class='btn btn-primary' name='leavecheck' value='{$leave['sno']}'>Confirm</button></form></td>
                            <td><form action='../php/deleten&l.php' method='post'><button class='btn btn-danger' name='leavedel' value='{$leave['sno']}'>Delete</button></form></td>
                        </tr>";
                        $n++;
                        }
        ...
        <div class="row">
            <div id="leave" class="hidden">
                <h1>Leave Form For Doctor</h1>
                <form action="../php/leaveform.php" method="POST" enctype="multipart/form-data" >
                    <div class="input-group mb-2">
                        <input type="text" class="form-control form-control-lg bg-light fs-6" aria-describedby="loca" name="depid" value="<?php echo $row['Host_dep'];?>" readonly>
                        <div class="input-group-text bg-primary text-white" id="loca" >
                            <span class="material-symbols-outlined">pin_drop</span>
                        </div>
                    </div>
                    <div class="input-group mb-2">
                        <input type="text" class="form-control form-control-lg bg-light fs-6" aria-describedby="loca" name="doctorid" value="<?php echo $row['sno'];?>" readonly>
                        <div class="input-group-text bg-primary text-white" id="loca" >
                            <span class="material-symbols-outlined">pin_drop</span>
                        </div>
                    </div>
                    <div class="input-group mb-2">
                        <input type="number" class="form-control form-control-lg bg-light fs-6" aria-describedby="loca" name="days" placeholder="Add Location name" required>
                        <div class="input-group-text bg-primary text-white" id="loca" >
                            <span class="material-symbols-outlined">pin_drop</span>
                        </div>
                    </div>
                    <div class="input-group mb-2">
                        <input type="date" class="form-control form-control-lg bg-light fs-6" aria-describedby="date" name="date1" placeholder="Start Date" required>
                        <div class="input-group-text bg-primary text-white" id="date" >
                            <span class="material-symbols-outlined">calendar_month</span>
                        </div>
                    </div>
                    <div class="input-group mb-2">
                        <input type="date" class="form-control form-control-lg bg-light fs-6" aria-describedby="date" name="date2" placeholder="End Date" required>
                        <div class="input-group-text bg-primary text-white" id="date" >
                            <span class="material-symbols-outlined">calendar_month</span>
                        </div>
                    </div>
                    <div class="input-group mb-2 me-2">
                        <select class="form-control form-control-lg bg-light fs-6" aria-label="Default select example" aria-describedby="dep" name="time" required>
                            <option selected disabled>Select Time Mode</option>
                            <option value="M">Morning</option>
                            <option value="A">Afternoon</option>
                            <option value="W">Whole Day</option>
                        </select>
                    ...
                    <div class="input-group mb-2">
                        <textarea cols="30" rows="6" class="form-control form-control-lg bg-light fs-6" aria-describedby="msg" name="text" placeholder="Short message for patients" required></textarea>
