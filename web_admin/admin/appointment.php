<?php
  ...
  if (isset($_SESSION["AEmail"])){
    ...
    // $appoint="SELECT * FROM `user_appointment` WHERE `Dep_id`='{$row['Host_dep']}' AND `Doc_id`='{$row['sno']}' AND `checkb`='0' AND `Tmode`='M'";
    // $appointres=mysqli_query($con,$appoint);
    // $app=mysqli_fetch_assoc($appointres);
    // $adate=$app['Appoint_date'];
...
<section>
    <div class="container">
        <div class="row">
            <div>
                <h1>Today's Morning Appointment</h1>
                <table class="table">
                          <thead>
                            <tr>
                              <th>Sno</th>
                          ...
                          <tbody>
                          <?php
                          $appoint="SELECT * FROM `user_appointment` WHERE `Dep_id`='{$row['Host_dep']}' AND `Doc_id`='{$row['sno']}' AND `checkb`='0' AND `Appoint_date`='$currentdate' AND `Tmode`='M' ORDER BY `Appoint_date` DESC";
                          $appointres=mysqli_query($con,$appoint);
                          
                          $n=1;
                            while($appointrow=mysqli_fetch_assoc($appointres)){
                                echo "
                            <tr>
                                <td>$n</td>
                                <td>{$appointrow['PatientN']}</td>
                                <td>{$appointrow['Page']}</td>
                                <td>{$appointrow['Pemail']}</td>
                                <td>{$appointrow['Pphone']}</td>
                                <td>{$appointrow['Appoint_date']}</td>
                                <td><form action='../php/check.php' method='post'><button name='appointcom' class='btn btn-danger' value='{$appointrow['Asno']}'>Click To Complete</button></form></td>
                            </tr>";
                            $n++;
                            }
                            ?>
            ...
            <div>
                <h1>Today's Afternoon Appointment</h1>
                    <table class="table">
                      <thead>
                        <tr>
                          <th>Sno</th>
                      ...
                      <tbody>
                      <?php
                      $appoint="SELECT * FROM `user_appointment` WHERE `Dep_id`='{$row['Host_dep']}' AND `Doc_id`='{$row['sno']}' AND `checkb`='0' AND `Appoint_date`='$currentdate' AND `Tmode`='A' ORDER BY `Appoint_date` DESC";
                      $appointres=mysqli_query($con,$appoint);
                        $n=1;
                        while($appointrow=mysqli_fetch_assoc($appointres)){
                            echo "
                        <tr>
                            ...
                            <td>{$appointrow['Pphone']}</td>
                            <td>{$appointrow['Appoint_date']}</td>
                            <td><form action='../php/sheck.php' method='post'><button name='appointcom' class='btn btn-danger' value='{$appointrow['Asno']}'>Click To Complete</button></form></td>
                        </tr>";
                        $n++;
                        }
